...
//파싱구조 변경 테스트중
function safeParseJson(text: string, stage: string) {
  try {
    const cleaned = text
      .replace(/```json/g, '')
      .replace(/```/g, '')
    ...
    console.error(`❌ ${stage} 파싱 실패 원본:\n`, text.slice(0, 500))
    throw new Error(`${stage} JSON 파싱 실패`)
  }
}
...
ipcMain.handle('analyze-image-staged', async (event, { imageData, mediaType }) => {
  ...
  try {
    ...
    if (analysisCache.overviewRaw && analysisCache.overview) {
      s1Raw = analysisCache.overviewRaw
      s1Parsed = analysisCache.overview
    } else {
      s1Raw = await geminiVision(imageData, mediaType, OVERVIEW_PROMPT, STAGE1_MAX_TOKENS)
      s1Parsed = safeParseJson(s1Raw, 'Stage1')

      analysisCache.overviewRaw = s1Raw
      analysisCache.overview = s1Parsed
    }
    ...
    if (analysisCache.zonesRaw) {
      s2Raw = analysisCache.zonesRaw
    } else {
      s2Raw = await geminiVision(
        imageData,
        mediaType,
        buildZonesPrompt(s1Raw), // ✅ raw 사용
        STAGE2_MAX_TOKENS
      )

      analysisCache.zonesRaw = s2Raw
    }
    ...
    if (analysisCache.zoneElementsRaw && analysisCache.zoneElements) {
      s3Raw = analysisCache.zoneElementsRaw
      s3Parsed = analysisCache.zoneElements
    } else {
      s3Raw = await geminiVision(
        imageData,
        mediaType,
        buildElementsPrompt(s1Raw, s2Raw), // ✅ raw 사용
        STAGE3_MAX_TOKENS
      )

      s3Parsed = safeParseJson(s3Raw, 'Stage3')

      analysisCache.zoneElementsRaw = s3Raw
      analysisCache.zoneElements = s3Parsed
    }
    ...
    const config = {
      ...
      elements: (s3Parsed.elements ?? []).map((el: any, i: number) => ({
        ...el,
        id: el.id ?? `el-${i + 1}`,
        value: el.value ?? '0',
        unit: el.unit ?? '',
    ...
    send(
      failedStage,
      ['전체 파악', '영역 분할', '요소 추출', 'JSON 조합'][failedStage - 1] ?? '알 수 없음',
      'error'
    )
